...
public class LoggerInterceptor implements HandlerInterceptor
{
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object o, Exception e) throws Exception {
        ...
        //获取本次请求日志实体
        CoreLogger loggerEntity = (CoreLogger) request.getAttribute(LOGGER_ENTITY);
        //设置请求时间差
        loggerEntity.setTimeConsuming(Integer.valueOf((currentTime - time)+""));
        //设置返回时间
        loggerEntity.setReturmTime(currentTime + "");
        //设置返回错误码
        ...
        loggerEntity.setReturnData(JSON.toJSONString(request.getAttribute(LoggerUtils.LOGGER_RETURN),
                ...
                SerializerFeature.WriteMapNullValue));
        //执行将日志写入数据库
        CoreLoggerDao coreLoggerDao = getDAO(CoreLoggerDao.class,request);
        coreLoggerDao.insert(loggerEntity);
    }
