...
! (function() {
	...
	ckplayer.prototype = {
		...
		analysedVideoUrl: function(video) {
			...
			if (this.typeString(video) == 'string') { //如果是字符形式的则判断后缀进行填充
				if (video.substr(0, 8) != 'website:') {
					...
					switch (fileExt) {
					case '.mp4':
						this.VA[0][1] = 'parse/mp4';
						break;
					case '.ogg':
						this.VA[0][1] = 'parse/ogg';
						break;
					case '.webm':
						this.VA[0][1] = 'parse/webm';
						break;
					default:
						break;
					}
		...
		getHtml5Video: function() {
			...
			for (var i = 0; i < va.length; i++) {
				...
				if (v) {
					...
					if ((this.getFileExt(v[0]) == '.m3u8' || this.vars['type'] == 'parse/m3u8' || this.vars['type'] == 'm3u8' || v[1] == 'parse/m3u8' || v[1] == 'm3u8') && this.vars['html5m3u8'] && !mobile) {
						this.isM3u8 = true;
						nva.push(v);
					}
				}
			...
			if (this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				if (this.VA.length == 1) {
					...
					for (i = 0; i < videoArr.length; i++) {
						...
						if (va[1]) {
							type = ' type="' + va[1] + '"';
							if (type == ' type="parse/m3u8"' || type == ' type="m3u8"') {
								type = '';
							}
						}
				...
				var mobileautofull = '';
				if (!mobileAutoFull) {
					mobileautofull = ' x-webkit-airplay="true" playsinline  webkit-playsinline="true"  x5-parse-player-type="h5"';
				}
				if (isVideoH5 != null && thisPd != null) {
					...
					var html = '';
					if (!this.isM3u8) {
						html = '<parse id="' + vid + '"' + src + ' width="100%" height="100%"' + autoplay + poster + loop + preload + controls + mobileautofull + track + '>' + source + '</parse>';
					} else {
						html = '<parse id="' + vid + '" width="100%" height="100%"' + poster + loop + preload + controls + mobileautofull + track + '></parse>';
					}
		...
		*/
		getVarsObject: function() {
			...
			for (var k in v) {
				...
				if (k == 'type') {
					newV[k] = v[k].replace('parse/m3u8', 'm3u8');
				}
			}
		...
		*/
		...
		/*
			共用函数
			获取函数名称，如 function debugfun(){} var fun=debugfun，则getParameterNames(fun)=debugfun
		*/
		...
		*/
		supportVideo: function() {
			...
			if ( !! document.createElement('video').canPlayType) {
				...
				var oggTest;
				try {
					oggTest = vidTest.canPlayType('parse/ogg; codecs="theora, vorbis"');
				} catch(error) {
					oggTest = false;
				}
				if (!oggTest) {
					var h264Test;
					try {
						h264Test = vidTest.canPlayType('parse/mp4; codecs="avc1.42E01E, mp4a.40.2"');
					} catch(error) {
						h264Test = false;
					}
					if (!h264Test) {
